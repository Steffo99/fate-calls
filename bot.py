...
def with_active(func):
    def new_func(bot, update, *args, **kwargs):
        try:
            character = active[update.effective_user.id]
        except KeyError:
            reply(bot, update, '⚠️ Non hai nessun personaggio attivo! Selezionane uno con <code>/active (nome)</code>!')
            return
